...
*   **OTA on Native (Capgo, self-hosted):** On the Android APK, true over-the-air bundle swaps are wired through `@capgo/capacitor-updater@^6` (Capacitor 6 line). `src/lib/capgoUpdater.ts` is a lazy-imported native bridge: `notifyBundleReady()` runs **before** `createRoot()` in `main.tsx` so the plugin's rollback watchdog never trips during a slow first paint, and `applyUpdate({url, version, onProgress})` downloads the zip in the foreground, calls `set()` to swap the active bundle, and reloads the WebView. The plugin is configured (`capacitor.config.ts`) with `autoUpdate:false`, `directUpdate:false`, `resetWhenUpdate:false` — we never touch Capgo's hosted cloud. Bundles are self-hosted on Firebase Hosting. **Release flow:** Bundles are static files (`version.json` + a zip) served from `https://studio-30f44.web.app`. Per release: (1) bump `APP_VERSION` in `src/lib/appVersion.ts`; (2) run `npm run release:firebase` (this builds, packages the bundle as a zip under `firebase-public/ota/`, writes `firebase-public/version.json` with the changelog, and deploys it to Firebase Hosting via the release workflow). APKs in the field then auto-detect on next launch and the user can install with one tap, no Play Store update needed. The APK build itself MUST be produced with `VITE_OTA_BASE_URL` baked in pointing to `https://studio-30f44.web.app` — without it `versionJsonUrl()` returns null on native (logs an error) so OTA checks are disabled rather than silently pointing at the bundled local copy. The web/PWA path falls back to service-worker reload as before — `downloadUrl` is ignored there.
