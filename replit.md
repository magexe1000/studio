...
*   **OTA on Native (Capgo, self-hosted):** On the Android APK, true over-the-air bundle swaps are wired through `@capgo/capacitor-updater@^6` (Capacitor 6 line). `src/lib/capgoUpdater.ts` is a lazy-imported native bridge: `notifyBundleReady()` runs **before** `createRoot()` in `main.tsx` so the plugin's rollback watchdog never trips during a slow first paint, and `applyUpdate({url, version, onProgress})` downloads the zip in the foreground, calls `set()` to swap the active bundle, and reloads the WebView. The plugin is configured (`capacitor.config.ts`) with `autoUpdate:false`, `directUpdate:false`, `resetWhenUpdate:false` — we never touch Capgo's hosted cloud. Bundles are self-hosted on the same Replit deployment that serves the web PWA. **Release flow (host-agnostic):** Bundles are static files (`version.json` + a zip) — host them anywhere with a public HTTPS URL. Default recommended host is **GitHub Pages from `main:/docs`** (no Replit dependency). One-time: enable Pages in the repo settings, build the APK once with `VITE_OTA_BASE_URL=<pages-url>` baked in. Per release: (1) bump `APP_VERSION` + `APP_CHANGELOG`; (2) `OTA_BASE_URL=<pages-url> pnpm --filter @workspace/chord-app release:gh-pages` (this builds, zips the bundle, and mirrors `version.json` + `bundles/bundle-<version>.zip` into the repo's `docs/` folder); (3) `git add docs && git commit && git push`. GitHub Pages auto-publishes within ~1 min. Old Replit/Replit-deployment alternative still works — pass `OTA_BASE_URL=https://<your-deployment>.replit.app pnpm bundle:publish` (no `OTA_OUTPUT_DIR`) and redeploy. APKs in the field then auto-detect on next launch and the user can install with one tap, no Play Store update needed. The APK build itself MUST be produced with `VITE_OTA_BASE_URL=<same URL>` baked in — without it `versionJsonUrl()` returns null on native (logs an error) so OTA checks are disabled rather than silently pointing at the bundled local copy. The web/PWA path falls back to service-worker reload as before — `downloadUrl` is ignored there.
