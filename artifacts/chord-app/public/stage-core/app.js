...
function openFeedbackMail() {
  if (_fbMailtoUrl) window.open(_fbMailtoUrl, '_blank');
}
...
function handleFeedbackBackdropClick(e) {
  if (e.target === document.getElementById('feedback-backdrop')) closeFeedbackModal();
}
function buildFeedbackBody() {
  var tried    = document.getElementById('fb-tried').value.trim() || '(not provided)';
  var wrong    = document.getElementById('fb-wrong').value.trim() || '(not provided)';
  var severity = document.getElementById('fb-severity').value;
  var device   = document.getElementById('fb-device').value;
  var email    = document.getElementById('fb-email').value.trim() || '(not provided)';
...
function prepareFeedback() {
  _fbEmailBody = buildFeedbackBody();
  _fbMailtoUrl = 'https://mail.google.com/mail/?view=cm&to=stagecore.contact%40gmail.com&su=' + encodeURIComponent('Stage Core App Feedback') + '&body=' + encodeURIComponent(_fbEmailBody);
  document.getElementById('fb-mailto-link').href = _fbMailtoUrl;
  document.getElementById('fb-body-form').style.display = 'none';
  document.getElementById('fb-footer-form').style.display = 'none';
  document.getElementById('fb-body-ready').style.display = 'block';
  document.getElementById('fb-footer-ready').style.display = 'flex';
  document.getElementById('fb-preview-text').value = _fbEmailBody;
}
function copyFeedbackText() {
  var ta = document.getElementById('fb-preview-text');
  ta.select();
  ta.setSelectionRange(0, 99999);
  navigator.clipboard.writeText(_fbEmailBody).then(function() {
...
function resetFeedbackForm() {
  document.getElementById('fb-body-form').style.display = 'flex';
  document.getElementById('fb-footer-form').style.display = 'flex';
  document.getElementById('fb-body-ready').style.display = 'none';
  document.getElementById('fb-footer-ready').style.display = 'none';
}
...
let _cloudAsTimer  = null;
let _authFirstCall = true;

// Called by cloud.js module via window.onSCAuthChange after auth state change
window.onSCAuthChange = function(user) {
  if (_authFirstCall) {
    _authFirstCall = false;
    if (!user) setTimeout(maybeShowWelcome, 800);
    else localStorage.setItem('sc_welcomed', '1');
  }
  if (user) {
    offerCloudRestore();
  }
...
function lmAccountAction() {
  closeLogoMenu();
  openAccountPanel();
}

function openAccountPanel() {
  const user = window.SC?.getUser?.();
  // Toggle logged-in/out views
  document.getElementById('ap-logged-out').style.display = user ? 'none' : 'block';
  document.getElementById('ap-logged-in').style.display  = user ? 'block' : 'none';
  if (user) {
    ...
    document.getElementById('ap-email-text').textContent = user.email;
    document.getElementById('ap-name-input').value      = user.displayName || '';
    const vb  = document.getElementById('ap-verified-badge');
    const uvb = document.getElementById('ap-unverified-badge');
    if (vb)  vb.style.display  = verified ? 'inline-flex' : 'none';
    if (uvb) uvb.style.display = verified ? 'none' : 'inline-flex';
    // verify section
    const ok      = document.getElementById('ap-verify-ok');
    const pending = document.getElementById('ap-verify-pending');
    if (ok)      ok.style.display      = verified ? 'block' : 'none';
    if (pending) pending.style.display = verified ? 'none'  : 'block';
  }
  document.getElementById('account-panel').classList.add('open');
  document.getElementById('account-panel-backdrop').classList.add('open');
}

function closeAccountPanel() {
  document.getElementById('account-panel').classList.remove('open');
  document.getElementById('account-panel-backdrop').classList.remove('open');
}

async function apGoogleSignIn() {
  if (!window.SC?.isReady()) { showToast('Firebase not ready — try again in a moment'); return; }
  try {
    await window.SC.signInWithGoogle();
    closeAccountPanel();
  } catch(e) {
...
async function apSaveName() {
  const val = document.getElementById('ap-name-input').value.trim();
  if (!val) { showToast('Enter a display name first'); return; }
  if (!window.SC?.updateDisplayName) { showToast('Not available'); return; }
  try {
    await window.SC.updateDisplayName(val);
    document.getElementById('ap-avatar').textContent = val.slice(0,2).toUpperCase();
    document.getElementById('ap-name').textContent   = val;
    // also refresh the logo menu badge
    const lmName = document.getElementById('lm-display-name');
    if (lmName) lmName.textContent = val;
    showToast('Display name updated');
  } catch(e) {
    showToast('Failed to update name: ' + (e.message || e.code));
  }
}

async function apResendVerify() {
  if (!window.SC?.resendVerificationEmail) { showToast('Not available'); return; }
  try {
    await window.SC.resendVerificationEmail();
    showToast('Verification email sent — check your inbox');
  } catch(e) {
    showToast('Failed to send: ' + (e.message || e.code));
  }
}

async function lmGoogleSignIn() {
  closeLogoMenu();
  if (!window.SC) { showToast('Loading… wait a moment and try again'); return; }
  if (!window.SC.isReady()) {
    showToast('Firebase not ready — check console for details');
    console.error('[SC] isReady=false. auth module may have failed to init.');
    return;
  }
  try {
    await window.SC.signInWithGoogle();
  } catch(e) {
    console.error('[SC] Google sign-in error:', e.code, e.message);
    if (e.code === 'auth/popup-blocked' || e.code === 'auth/popup-closed-by-user') {
      showToast('Popup blocked — use Sign In button instead');
      setTimeout(() => openAuthModal('login'), 400);
    } else if (e.code === 'auth/unauthorized-domain') {
      showToast('Domain not authorized in Firebase — add it in Auth settings');
    } else if (e.code !== 'auth/popup-closed-by-user') {
      showToast(_friendlyAuthError(e.code || e.message || ''));
    }
  }
}
...
function switchAuthTab(tab) {
  ...
  document.getElementById('auth-footer-text').textContent    = isLogin ? 'New to Stage Core? ' : 'Already have an account? ';
  document.getElementById('auth-footer-switch').textContent  = isLogin ? 'Create Account' : 'Sign In';
  document.getElementById('auth-error').textContent          = '';
...
async function submitAuth() {
  ...
  if (!window.SC?.isReady()) {
    errEl.textContent = 'Cloud not configured — add VITE_FIREBASE_* env vars.';
    return;
  }
  btn.disabled = true;
  try {
    if (currentAuthTab === 'login') {
      await window.SC.signIn(email, password);
    } else {
      if (password.length < 6) { errEl.textContent = 'Password must be at least 6 characters.'; btn.disabled = false; return; }
      await window.SC.signUp(email, password);
      showToast('Welcome! Check your email to verify your account.');
    }
    closeAuthModal();
  } catch(e) {
    if (e.code === 'auth/multi-factor-auth-required') {
      _mfaResolver = window.SC.getMfaResolver(e);
      closeAuthModal();
      openMfaModal();
    } else {
      errEl.textContent = _friendlyAuthError(e.code || e.message || '');
    }
  }
  btn.disabled = false;
}
async function submitGoogleAuth() {
  ...
  errEl.textContent = '';
  if (!window.SC?.isReady()) {
    errEl.textContent = 'Firebase not ready. Check that Email/Password is enabled in Firebase Auth.';
    return;
  }
  btn.disabled = true;
  try {
    ...
    if (e.code === 'auth/popup-blocked') {
      errEl.textContent = 'Popup was blocked by your browser.';
    } else if (e.code === 'auth/unauthorized-domain') {
...
async function doSignOut() {
  ...
  if (window.SC) await window.SC.logOut();
  showToast('Signed out');
}

// ── Quick verify-email from logo menu badge ────────────────
async function resendVerifyEmail() {
  try {
    await window.SC.resendVerificationEmail();
    showToast('Verification email sent — check your inbox');
  } catch(e) { showToast('Could not send email — try again shortly'); }
}
...
function openAccountModal() {
  const user = window.SC?.getUser();
  if (!user) return;
  document.getElementById('acm-display-name').value = user.displayName || '';
  const verified = user.emailVerified;
  document.getElementById('acm-verify-status').textContent =
    verified ? '✓ Your email is verified.' : 'Your email is not verified yet.';
  document.getElementById('acm-verify-status').style.color = verified ? '#00dc64' : '#ffb400';
  document.getElementById('acm-resend-btn').style.display  = verified ? 'none' : '';
  _refreshAcm2faStatus();
  document.getElementById('acm-msg').textContent = '';
  document.getElementById('acm-msg').className   = 'acm-msg';
...
function _refreshAcm2faStatus() {
  const has2fa  = window.SC?.isTotpEnrolled?.() || false;
  const statusEl = document.getElementById('acm-2fa-status');
  const btnEl    = document.getElementById('acm-2fa-btn');
  if (statusEl) statusEl.textContent = has2fa ? '2FA is enabled.' : '2FA is not enabled.';
  if (statusEl) statusEl.style.color = has2fa ? '#00dc64' : '#767575';
  if (btnEl)    { btnEl.textContent = has2fa ? 'Disable 2FA' : 'Enable 2FA'; btnEl.className = has2fa ? 'acm-btn danger' : 'acm-btn secondary'; }
}
function _acmMsg(text, type) {
  const el = document.getElementById('acm-msg');
  el.textContent = text; el.className = 'acm-msg ' + type;
  if (text) setTimeout(() => { if (el.textContent === text) el.textContent = ''; }, 4000);
}
async function saveDisplayName() {
  const val = document.getElementById('acm-display-name').value.trim();
  if (!val) { _acmMsg('Enter a display name.', 'err'); return; }
  try {
    await window.SC.updateDisplayName(val);
    document.getElementById('lm-display-name').textContent = val;
    _acmMsg('Display name updated.', 'ok');
  } catch(e) { _acmMsg('Could not save. Try again.', 'err'); }
}
async function acmResendVerify() {
  try {
    await window.SC.resendVerificationEmail();
    _acmMsg('Verification email sent — check your inbox.', 'ok');
  } catch(e) { _acmMsg('Could not send email. Try again shortly.', 'err'); }
}
async function acmToggle2fa() {
  const has2fa = window.SC?.isTotpEnrolled?.() || false;
  if (has2fa) {
    showConfirm('Disable two-factor authentication? Your account will be less secure.', async () => {
      try {
        await window.SC.unenrollTotp();
        _refreshAcm2faStatus();
        _updateLm2faBadge();
        _acmMsg('2FA has been disabled.', 'ok');
      } catch(e) { _acmMsg('Could not disable 2FA. Try again.', 'err'); }
    });
  } else {
    closeAccountModal();
    await openTotpModal();
...
function _updateLm2faBadge() {
  const badge2fa = document.getElementById('lm-2fa-badge');
  if (!badge2fa) return;
  const has2fa = window.SC?.isTotpEnrolled?.() || false;
  badge2fa.textContent = has2fa ? '2FA On' : '2FA Off';
  badge2fa.className   = 'lm-2fa-badge ' + (has2fa ? 'on' : 'off');
}

// ── Danger zone ────────────────────────────────────────────
function handleDisableAccount() {
  showConfirm(
    'Disable your account? You will be signed out. Contact support to re-enable.',
    async () => {
      try {
        await window.SC.disableAccount();
        showToast('Account disabled. You have been signed out.');
      } catch(e) {
        if (e.code === 'auth/requires-recent-login') {
          showToast('Please sign out and sign back in first, then try again.');
        } else {
          showToast('Could not disable account. Try again.');
        }
      }
    }
  );
}
function handleDeleteAccount() { openDeleteAccountModal(); }

function openDeleteAccountModal() {
  const user = window.SC?.getUser?.();
  if (!user) { showToast('Not signed in.'); return; }
  const email = user.email || '';
  // Close account panel first if open
  if (typeof closeAccountPanel === 'function') closeAccountPanel();
  // Show the email hint and reset input
  const hint = document.getElementById('da-email-hint');
  if (hint) hint.textContent = email;
  const inp = document.getElementById('da-confirm-input');
  if (inp) { inp.value = ''; inp.placeholder = email; }
...
function validateDeleteInput() {
  ...
  const btn   = document.getElementById('da-confirm-btn');
  if (!inp || !btn) return;
  const match = inp.value.trim().toLowerCase() === email.toLowerCase();
  btn.disabled = !match;
  btn.style.opacity  = match ? '1'        : '0.45';
  btn.style.cursor   = match ? 'pointer'  : 'not-allowed';
  btn.style.background = match ? '#ef4444' : '#7f1d1d';
  btn.style.color    = match ? '#fff'     : '#fca5a5';
}
...
async function confirmDeleteAccount() {
  ...
  try {
    await window.SC.deleteAccount();
    closeDeleteAccountModal();
    showToast('Account permanently deleted.');
    localStorage.removeItem('sc_welcomed');
  } catch(e) {
    ...
    if (e.code === 'auth/requires-recent-login') {
      ...
      console.error('[SC] Delete account error:', e);
    }
  }
}

// ── TOTP enrollment modal ─────────────────────────────────
let _totpSecretObj = null;
async function openTotpModal() {
  document.getElementById('totp-error').textContent  = '';
  document.getElementById('totp-otp-input').value    = '';
  document.getElementById('totp-qr-img').src         = '';
  document.getElementById('totp-secret-key').textContent = 'Generating…';
  document.getElementById('totp-modal-backdrop').classList.add('open');
  try {
    const { totpSecret, qrUrl, secretKey } = await window.SC.generateTotpSecret();
    _totpSecretObj = totpSecret;
    const encodedUrl = encodeURIComponent(qrUrl);
    document.getElementById('totp-qr-img').src = `https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodedUrl}`;
    document.getElementById('totp-secret-key').textContent = secretKey;
  } catch(e) {
    document.getElementById('totp-error').textContent = 'Could not generate QR code. Are you signed in?';
    console.error('[SC] TOTP generate error:', e);
  }
}
function closeTotpModal() {
  document.getElementById('totp-modal-backdrop').classList.remove('open');
  _totpSecretObj = null;
}
async function confirmTotpEnroll() {
  const otp = document.getElementById('totp-otp-input').value.replace(/\s/g,'');
  const errEl = document.getElementById('totp-error');
  const btn   = document.getElementById('totp-confirm-btn');
  errEl.textContent = '';
  if (otp.length !== 6 || !/^\d+$/.test(otp)) { errEl.textContent = 'Enter the 6-digit code from your app.'; return; }
  if (!_totpSecretObj) { errEl.textContent = 'Session expired — close and try again.'; return; }
  btn.disabled = true;
  try {
    await window.SC.enrollTotp(_totpSecretObj, otp);
    closeTotpModal();
    _updateLm2faBadge();
    ...
    console.error('[SC] TOTP enroll error:', e);
  }
  btn.disabled = false;
}
...
function openMfaModal() {
  ...
  document.getElementById('mfa-modal-backdrop').classList.add('open');
  setTimeout(() => document.getElementById('mfa-otp-input').focus(), 100);
}
function closeMfaModal() {
  document.getElementById('mfa-modal-backdrop').classList.remove('open');
  _mfaResolver = null;
}
async function submitMfa() {
  const otp   = document.getElementById('mfa-otp-input').value.replace(/\s/g,'');
  const errEl = document.getElementById('mfa-error');
  const btn   = document.getElementById('mfa-submit-btn');
  errEl.textContent = '';
  if (otp.length !== 6 || !/^\d+$/.test(otp)) { errEl.textContent = 'Enter the 6-digit code from your app.'; return; }
  if (!_mfaResolver) { errEl.textContent = 'Session expired. Please sign in again.'; return; }
  btn.disabled = true;
  try {
    await window.SC.resolveSignInMfa(_mfaResolver, otp);
    closeMfaModal();
  } catch(e) {
    errEl.textContent = e.code === 'auth/invalid-verification-code'
...
// ── Cloud indicator ───────────────────────────────────────
function setCloudIndicator(status) {
  const el = document.getElementById('cloud-save-indicator');
  if (!el) return;
  el.className = 'visible desktop-only ' + status;
  const labels = { saving: 'Saving…', saved: 'Saved', error: 'Error' };
  document.getElementById('csi-text').textContent = labels[status] || 'Cloud';
  if (status === 'saved') {
    setTimeout(() => {
      el.className = 'visible desktop-only';
      document.getElementById('csi-text').textContent = 'Cloud';
    }, 2500);
  }
}
...
function maybeShowWelcome() {
  ...
  if (window.SC?.getUser()) { localStorage.setItem('sc_welcomed','1'); return; }
...
function dismissWelcome(action) {
  ...
  if (action === 'signin') setTimeout(() => openAuthModal('login'), 200);
}

function scheduleCloudAutosave() {
  if (!window.SC?.isReady() || !window.SC.getUser()) return;
  clearTimeout(_cloudAsTimer);
  _cloudAsTimer = setTimeout(async () => {
    try {
      setCloudIndicator('saving');
      await window.SC.cloudAutoSave(getCloudState());
...
async function saveCloud() {
  if (!window.SC?.isReady()) { showToast('Cloud not configured'); return; }
  if (!window.SC.getUser())  { showToast('Sign in to save to cloud'); return; }
  try {
    setCloudIndicator('saving');
    await window.SC.cloudAutoSave(getCloudState());
    setCloudIndicator('saved');
    ...
    setCloudIndicator('error');
    showToast('Cloud save failed: ' + (e.message || 'unknown'));
  }
}
...
function openCloudModal() {
  document.getElementById('user-dropdown').classList.remove('open');
  document.getElementById('cloud-modal-backdrop').classList.add('open');
  loadCloudProjectsList();
  setTimeout(() => document.getElementById('cloud-save-name').focus(), 100);
}
function closeCloudModal() {
  document.getElementById('cloud-modal-backdrop').classList.remove('open');
}

// ── Pinned projects stored locally ─────────────────────────
const PINNED_KEY = 'scPinnedProjects';
function _getPinned() {
...
let _cloudProjectsCache = [];

async function loadCloudProjectsList() {
  const listEl  = document.getElementById('cloud-projects-list');
  const emptyEl = document.getElementById('cloud-list-empty');
  listEl.querySelectorAll('.cp-row').forEach(r => r.remove());
  emptyEl.textContent = 'Loading…';
  emptyEl.style.display = 'block';
  try {
    const projects = await window.SC.cloudListProjects();
    _cloudProjectsCache = projects;
    _renderFilteredProjects();
  } catch(e) {
    emptyEl.textContent = 'Failed to load: ' + (e.message || 'unknown error');
  }
}
...
function _renderFilteredProjects() {
  ...
  const sortEl   = document.getElementById('cp-sort');
  const query    = searchEl ? searchEl.value.toLowerCase().trim() : '';
  const sort     = sortEl ? sortEl.value : 'date';
  const pinned   = _getPinned();

  let projects = _cloudProjectsCache.filter(p =>
  ...
  if (!projects.length) {
    emptyEl.textContent = query ? 'No projects match your search' : 'No saved projects yet';
    emptyEl.style.display = 'block';
    return;
  }
  ...
  projects.forEach(p => {
    let dateStr = '—';
    try { dateStr = p.updatedAt?.toDate ? p.updatedAt.toDate().toLocaleDateString(undefined, { month:'short', day:'numeric', year:'numeric' }) : dateStr; } catch {}
    ...
    row.innerHTML = `
      <div style="min-width:0;overflow:hidden;flex:1;">
        <div class="cp-name" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
          ${isPinned ? '<span style="color:#ffd700;margin-right:4px;font-size:11px;">★</span>' : ''}${p.name}
        </div>
        <div class="cp-date">${dateStr}</div>
      </div>
      <div class="cp-actions">
        <button class="cp-pin-btn${isPinned ? ' pinned' : ''}" onclick="togglePinProject('${p.id}')" title="${isPinned ? 'Unpin' : 'Pin'}">${isPinned ? '★' : '☆'}</button>
        <button class="cp-dup-btn" onclick="duplicateCloudProject('${p.id}','${safeName}')" title="Duplicate">Dup</button>
        <button class="cp-load-btn" onclick="loadCloudProject('${p.id}')">Load</button>
        <button class="cp-del-btn"  onclick="deleteCloudProject('${p.id}','${safeName}')">Del</button>
      </div>`;
    listEl.appendChild(row);
  });
}

async function duplicateCloudProject(projectId, name) {
  try {
    const proj = await window.SC.cloudLoadProject(projectId);
    if (!proj?.data) { showToast('Could not load project data'); return; }
    const newId   = 'p_' + Date.now();
    const newName = name + ' (copy)';
    await window.SC.cloudSave(newId, newName, proj.data);
    showToast('Duplicated as "' + newName + '"');
    loadCloudProjectsList();
  } catch(e) {
    showToast('Duplicate failed: ' + (e.message || 'unknown'));
  }
}

// ── State snapshot for cloud ──────────────────────────────
...
async function saveToCloud() {
  const nameEl = document.getElementById('cloud-save-name');
  ...
  try {
    ...
    await window.SC.cloudSave(projectId, name, getCloudState());
    nameEl.value = '';
    setCloudIndicator('saved');
...
async function loadCloudProject(projectId) {
  showConfirm('Load this project from the cloud? Any unsaved local changes will be replaced.', async () => {
    try {
      const proj = await window.SC.cloudLoadProject(projectId);
      if (!proj?.data) { showToast('Project data is empty or missing'); return; }
      _applyCloudData(proj.data);
      closeCloudModal();
      showToast('Cloud project loaded ✓');
    } catch(e) {
      showToast('Load failed: ' + (e.message || 'unknown'));
    }
  });
}

async function deleteCloudProject(projectId, name) {
  showConfirm('Delete "' + name + '" from the cloud? This cannot be undone.', async () => {
    try {
      await window.SC.cloudDeleteProject(projectId);
      loadCloudProjectsList();
...
function _dataWeight(d) {
  if (!d) return 0;
  return (d.members?.length  || 0) * 10
       + (d.elements?.length || 0)
       + (d.setlist?.length  || 0)
       + (d.gear?.length     || 0)
       + (d.presets?.length  || 0) * 2
       + (d.riderNeeds?.length || 0);
}

async function offerCloudRestore() {
  if (!window.SC?.isReady() || !window.SC.getUser()) return;
  try {
    const as = await window.SC.cloudLoadAutoSave();
    if (!as?.data) return;
    const d = as.data;
    const cloudWeight = _dataWeight(d);
    if (!cloudWeight) return;
    const localRaw = localStorage.getItem('sc_session');
    if (!localRaw) {
      // No local session — always restore from cloud
      _applyCloudData(d);
      _sessionSave();
      showToast('Session restored from cloud ✓');
      return;
    }
    let localData;
    try { localData = JSON.parse(localRaw); } catch { localData = {}; }
    const localWeight = _dataWeight(localData);
    const cloudTime  = new Date(d.savedAt         || 0).getTime();
    const localTime  = new Date(localData.savedAt  || 0).getTime();
    // Apply cloud if it has more data (handles new additions from another device)
    // OR same data weight but cloud is newer (handles edits / value changes).
    // Never apply cloud if local has strictly more data — that protects against a
    // data-light device (e.g. mobile with 0 members) overwriting a richer local session.
    if (cloudWeight > localWeight || (cloudWeight === localWeight && cloudTime > localTime)) {
      _applyCloudData(d);
      _sessionSave();
      showToast('Session synced from cloud ✓');
    }
  } catch { /* silent — no autosave yet */ }
}

async function pullFromCloud() {
  if (!window.SC?.isReady()) { showToast('Cloud not configured'); return; }
  if (!window.SC.getUser())  { showToast('Sign in to sync from cloud'); return; }
  try {
    showToast('Checking cloud...');
    const as = await window.SC.cloudLoadAutoSave();
    if (!as?.data) {
      showToast('No cloud backup found. Save from your other device first.');
      return;
    }
    const d = as.data;
    const parts = [];
    if (d.members?.length)  parts.push(d.members.length + ' member' + (d.members.length !== 1 ? 's' : ''));
    if (d.setlist?.length)  parts.push(d.setlist.length + ' song' + (d.setlist.length !== 1 ? 's' : ''));
    if (d.gear?.length)     parts.push(d.gear.length + ' gear item' + (d.gear.length !== 1 ? 's' : ''));
    if (d.riderNeeds?.length) parts.push(d.riderNeeds.length + ' rider input' + (d.riderNeeds.length !== 1 ? 's' : ''));
    if (!parts.length) { showToast('Cloud backup has no member/setlist data.'); return; }
    showConfirm(
      'Cloud has: ' + parts.join(', ') + '. Sync to this device? (Stage plot elements are kept as-is.)',
      () => {
        // Merge non-canvas data only — stage elements stay local
        if (d.members)    { state.members = JSON.parse(JSON.stringify(d.members)); _memberNextId = state.members.reduce((mx,m)=>Math.max(mx,(parseInt(String(m.id).replace('m',''))||0)+1),1); }
        if (d.setlist)    state.setlist  = JSON.parse(JSON.stringify(d.setlist));
        if (d.segments)   state.segments = JSON.parse(JSON.stringify(d.segments));
        if (d.gear)       state.gear     = JSON.parse(JSON.stringify(d.gear));
        if (d.riderNeeds) { state.riderNeeds = JSON.parse(JSON.stringify(d.riderNeeds)); }
