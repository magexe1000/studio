...
export default function DrumEditor() {
  ...
  return (
    <div data-perf-mode={drumPrefs.performanceMode ? 'on' : undefined} style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--app-bg)', overflow: 'hidden', userSelect: 'none', WebkitUserSelect: 'none' }}>
      ...
      {/* ── Top bar ──────────────────────────────────────────────────────── */}
      <div style={{ flexShrink: 0, height: inEditor ? (isLandscape ? 40 : 52) : undefined, display: 'flex', alignItems: 'center', padding: isLandscape && inEditor ? '0 10px' : inEditor ? '0 20px' : '24px 20px 12px', gap: isLandscape && inEditor ? 6 : 8, background: 'var(--app-bg)', borderBottom: isLandscape && inEditor ? '1px solid rgba(128,128,128,0.06)' : 'none' }}>
        {inEditor ? (
          <>
