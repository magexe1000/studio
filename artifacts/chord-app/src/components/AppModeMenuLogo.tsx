...
export function AppModeMenuLogo({ color, size = 14 }: { color?: string; size?: number }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-flex' }}>
      ...
      >
        {currentMode === 'drums' ? <DrumexLogo size={size} /> : currentMode === 'stage' ? <StagexLogoIcon size={size} /> : currentMode === 'groovex' ? <GroovexLogo size={size} /> : currentMode === 'vocalex' ? <span style={{ display: 'inline-flex', transform: 'translateY(4px)' }}><VocalexLogo size={size + 3} /></span> : <ChordexLogo size={size} />}
